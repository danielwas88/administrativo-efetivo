...
CREATE TABLE `alertas` (
	...
	`tipo` enum('afastamento_vencendo','conflito_escala','efetivo_insuficiente','escala_nao_preenchida') NOT NULL,
...
--> statement-breakpoint
CREATE TABLE `config_horas_regime` (
	`id` int AUTO_INCREMENT NOT NULL,
	`regime` enum('08x40','12x36','12x60','24x72','expediente') NOT NULL,
	`horasNormaisPorDia` decimal(5,2) NOT NULL,
	`horasNormaisPorSemana` decimal(5,2) NOT NULL,
	`descricao` text,
	`ativo` boolean NOT NULL DEFAULT true,
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `config_horas_regime_id` PRIMARY KEY(`id`),
	CONSTRAINT `config_horas_regime_regime_unique` UNIQUE(`regime`)
);
--> statement-breakpoint
...
CREATE TABLE `militares` (
	...
	`email` varchar(320),
	`telefone` varchar(20),
	`posto` enum('soldado','cabo','sargento','tenente','capitao','major','tenente_coronel','coronel') NOT NULL,
	...
	`matriculaPmdf` varchar(20),
	`dataInclusaoCorporacao` date,
	`dataInclusaoFuncao` date,
	`dataInclusaoSecaoEsquadrao` date,
	`dataPromocaoPosto` date,
	`genero` enum('masculino','feminino','outro'),
	`cidadeResidencia` varchar(100),
	`contatoPlanosChamada` varchar(20),
	`contatoEmergenciaFamiliar` varchar(255),
	`restricaoMedica` text,
	`porteArma` enum('ativo','suspenso') NOT NULL DEFAULT 'ativo',
	`motivoSuspensaoArma` text,
	`dataSuspensaoArma` date,
	`validadeBienal` date,
...
--> statement-breakpoint
CREATE TABLE `tipos_funcao` (
	`id` int AUTO_INCREMENT NOT NULL,
	`nome` varchar(100) NOT NULL,
	`tipoEfetivo` enum('operacional','expediente','administrativo') NOT NULL,
	`descricao` text,
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `tipos_funcao_id` PRIMARY KEY(`id`),
	CONSTRAINT `tipos_funcao_nome_unique` UNIQUE(`nome`)
);
--> statement-breakpoint
ALTER TABLE `users` MODIFY COLUMN `role` enum('admin','gestor','visualizador','user') NOT NULL DEFAULT 'user';--> statement-breakpoint
...
CREATE INDEX `matricula_idx` ON `militares` (`matricula`);--> statement-breakpoint
CREATE INDEX `sublotacao_idx` ON `militares` (`sublotacao`);--> statement-breakpoint
CREATE INDEX `posto_idx` ON `militares` (`posto`);--> statement-breakpoint
CREATE INDEX `cidadeResidencia_idx` ON `militares` (`cidadeResidencia`);--> statement-breakpoint
CREATE INDEX `validadeBienal_idx` ON `militares` (`validadeBienal`);--> statement-breakpoint
CREATE INDEX `validadeIdentidade_idx` ON `militares` (`validadeIdentidadeMilitar`);--> statement-breakpoint
CREATE INDEX `validadeCnh_idx` ON `militares` (`validadeCnh`);--> statement-breakpoint
...
CREATE INDEX `restricao_data_fim_idx` ON `restricoes_medicas` (`dataFim`);